...
`okButtonProps` and `cancelButtonProps` support passing in the `initialFocus` parameter, which will automatically focus at this position when the panel is opened. Version 2.9.0 supported.

`content` supports function, and its parameter is an object, which binds `initialFocusRef` to the focusable DOM or component, and it will automatically focus at this position when the panel is opened. Version 2.9.0 supported.
...
| content            | Content displayed (function type, supported in version 2.9.0)                                                                                                         | ReactNode\|({ initialFocusRef }) => ReactNode        |                     |
